...
function renderInvoicePage(doc: jsPDF, invoice: InvoicePdfData) {
  ...
  y += 14;
  doc.setTextColor(37, 99, 235); // blue, matching the reference's due-date highlight
  doc.text(formatDate(invoice.dueDate), col1X, y);
  doc.setTextColor(0, 0, 0);

  y += 20;

  // Invoices created before line items existed have amountCents but no items -
  // fall back to a single summary row so the PDF isn't just a blank table.
  const rows =
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      // Thin bottom rule under every row, like the reference's line-separated table
  ...
  // Use the invoice's own amountCents rather than re-summing items here - it's
  // the server-computed source of truth, and some older invoices (created
  // before line items existed) have an amount but no items at all.
